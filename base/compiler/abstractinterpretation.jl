...
@inline function abstract_interpret_bb_stmt!(interp::AbstractInterpreter,
    pc_vars::VarTable, @nospecialize(stmt), frame::InferenceState)
...
@inline function abstract_interpret_return(interp::AbstractInterpreter,
    nslots::Int, node::ReturnNode, frame::InferenceState)
        ...
        for frame.currpc in frame.currpc:bbend
            ...
            if frame.currpc == bbend
                ...
                elseif isa(stmt, ReturnNode)
                    ssavaluetypes[frame.currpc] = Any
                    abstract_interpret_return(interp, nslots, stmt, frame)
                    @goto find_next_bb
            ...
            (; changes, type) = abstract_interpret_bb_stmt!(interp,
                frame.pc_vars, stmt, frame)::StmtResult
            if type === Union{}
                @goto find_next_bb
            end
            if changes !== nothing
